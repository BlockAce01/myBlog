...
async function setupAdmin() {
  try {
    ...
    if (!email) {
      ...
      console.log(
        'Example: node setup-admin.js admin@example.com admin "Admin User" your-token',
      );
    ...
    if (existingAdmin) {
      ...
      console.log(
        `   Has Public Key: ${existingAdmin.publicKey ? "Yes" : "No"}\n`,
      );
      console.log(
        "⚠️  Only one admin user is supported. Delete existing admin first if needed.",
      );
      process.exit(0);
    }
