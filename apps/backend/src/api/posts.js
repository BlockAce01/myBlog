...
router.post("/posts", authenticateToken, adminCrudOnly, async (req, res) => {
  try {
    ...
    const newPost = new BlogPost({
      ...
      status: status || "draft",
      scheduledPublishDate:
        status === "scheduled" ? new Date(scheduledPublishDate) : undefined,
    });
  ...
  try {
    ...
    if (version !== undefined && existingPost.version !== version) {
      return res.status(409).json({
        message:
          "Post has been modified by another user. Please refresh and try again.",
...
router.post("/posts/:postId/comments", async (req, res) => {
  try {
    ...
    if (!commentText) {
      return res
        .status(400)
        .json({ message: "Missing required field: commentText" });
    }
...
router.put("/posts/drafts/:id", authenticateToken, async (req, res) => {
  try {
    ...
    const updatedDraft = await BlogPost.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
