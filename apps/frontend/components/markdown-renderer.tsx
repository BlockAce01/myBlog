        ...
        components={{
          code({ node, className, children, ...props }) {
            ...
            if (lines.length > 0 && !isInline) {
              ...
              if (
                firstLine.includes("// show-line-numbers") ||
                firstLine.includes("// line-numbers")
              ) {
            ...
            if (!isInline && language) {
              return (
                <div
                  key={node?.position?.start?.offset || Math.random()}
                  className="my-4"
                >
                  <CodeBlock
                    code={actualCode}
          ...
          p({ children }) {
            return <p className="leading-7 mb-4 last:mb-0">{children}</p>;
          },
        }}
      >
        {content}
